package kst.ksti.chauffeur.ui.view;
...
public class RadiusImageView extends AppCompatImageView {
    ...
    private void initView(AttributeSet attrs) {
        ...
        if (typedArray != null) {
            for (int i = 0, j = typedArray.getIndexCount(); i < j; i++) {
                ...
                switch (attr) {
                    case R.styleable.RadiusImageView_radius:
                        radius = typedArray.getDimension(attr, 0);
                        break;
                    case R.styleable.RadiusImageView_borderWidth:
                        borderWidth = typedArray.getDimension(attr, 0);
                        break;
                    case R.styleable.RadiusImageView_borderColor:
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (borderWidth > 0) {
            ...
            paint.setStrokeWidth(Util.convertDpToPixel(borderWidth, getContext()));
    ...
    public void setRadius(float radius) {
        this.radius = Util.convertDpToPixel(radius, getContext());
    }
}